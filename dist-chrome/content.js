    ...
    webextension_polyfill__WEBPACK_IMPORTED_MODULE_0___default().storage.onChanged.addListener((changes, areaName) => {
        if (areaName === 'sync') {
            ...
            if (changes.soundEnabled) {
                soundEnabled = changes.soundEnabled.newValue ?? true;
            }
            if (changes.soundVolume) {
                soundVolume = changes.soundVolume.newValue ?? 0.5;
            }
        }
    });
 ...
 */
function checkMessage(messageElement) {
    // Extract username
    ...
    const matchedKeyword = keywords.find(keyword => lowerMessage.includes(keyword.toLowerCase()));
    if (matchedKeyword) {
