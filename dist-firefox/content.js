...
// Cache keywords in memory to avoid constant storage reads
let keywords = [];
/**
 * Initialize: Load keywords from storage when script starts
 */
async function init() {
    const data = await webextension_polyfill__WEBPACK_IMPORTED_MODULE_0___default().storage.sync.get(['keywords']);
    keywords = data.keywords || [];
    // Listen for storage changes (when user updates keywords in options)
