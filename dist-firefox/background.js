 ...
 */
webextension_polyfill__WEBPACK_IMPORTED_MODULE_0___default().runtime.onMessage.addListener(async (message) => {
    if (typeof message === 'object' && message !== null && message.type === 'KEYWORD_MATCH') {
        await handleKeywordMatch(message);
    }
});
/**
 * Handle keyword match: show notification and play sound
 */
async function handleKeywordMatch(data) {
    // Get settings
    const settings = await webextension_polyfill__WEBPACK_IMPORTED_MODULE_0___default().storage.sync.get(['soundEnabled', 'soundVolume']);
    const soundEnabled = settings.soundEnabled ?? true;
    const soundVolume = settings.soundVolume ?? 0.5;
    // Create desktop notification
    await webextension_polyfill__WEBPACK_IMPORTED_MODULE_0___default().notifications.create({
        type: 'basic',
        iconUrl: webextension_polyfill__WEBPACK_IMPORTED_MODULE_0___default().runtime.getURL('icons/icon-128.png'),
 ...
 */
webextension_polyfill__WEBPACK_IMPORTED_MODULE_0___default().browserAction.onClicked.addListener(() => {
...
webextension_polyfill__WEBPACK_IMPORTED_MODULE_0___default().runtime.onInstalled.addListener(async () => {
    const existing = await webextension_polyfill__WEBPACK_IMPORTED_MODULE_0___default().storage.sync.get(['keywords', 'soundEnabled', 'soundVolume']);
